...
async function toggleRunning() {
	...
	try {
		...
		showToast(
			`Failed to toggle watcher: ${error instanceof Error ? error.message : String(error)}`,
			'error',
		);
...
async function setDryRun(enabled: boolean) {
	try {
		...
		showToast(
			`Failed to set dry run: ${error instanceof Error ? error.message : String(error)}`,
			'error',
		);
...
async function setLaunchOnLogin(enabled: boolean) {
	try {
		...
		showToast(
			`Failed to update launch on login: ${error instanceof Error ? error.message : String(error)}`,
			'error',
		);
